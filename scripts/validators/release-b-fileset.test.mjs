...
describe("Release B file-set validator", () => {
	it("aggregates imports and renders across multiple TSX files", () => {
		const result = gradeFileSet(
			{
				"src/App.tsx":
					'import { Button as Action } from "@corensystem/coren-ui/button";\nexport {Panel} from "./Panel";\nexport const App=()=> <Action>Save</Action>;',
				"src/Panel.tsx": 'import { Card } from "@corensystem/coren-ui/card";\nexport const Panel=()=> <Card>Panel</Card>;',
			},
			{},
			catalog,
		);
	...
	it("reports hard-coded CSS values with category and location", () => {
		const result = gradeFileSet({
			"src/App.tsx": 'import { Button } from "@corensystem/coren-ui/button"; export const App=()=> <Button>Save</Button>;',
			"src/app.css":
	...
	it("treats Core variables as tokenized and supports narrow exceptions", () => {
		const result = gradeFileSet({
			"src/App.tsx": 'import { Card } from "@corensystem/coren-ui/card"; export const App=()=> <Card />;',
			"src/app.css": [
				".panel {",
