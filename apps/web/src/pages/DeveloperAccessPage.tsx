...
// connects to the WakeCore Hosted MCP: the tool-agnostic knowledge API. Card-based layout: install the
// Codex plugin → inspect the MCP endpoint → connect a specific client with a ready-to-paste setup prompt.
// The endpoint is env-configurable (VITE_WAKECORE_MCP_URL) so it shows the hosted URL once deployed.
...
// Same-origin by default: this page is served from core.wakecap.com, where /mcp is the hosted function.
const MCP_URL =
	readEnv("VITE_WAKECORE_MCP_URL") ??
	(typeof window === "undefined" ? "http://localhost:4100/mcp" : `${window.location.origin}/mcp`);
const MCP_BASE = MCP_URL.replace(/\/mcp\/?$/, "");
const MARKETPLACE_SOURCE = readEnv("VITE_WAKECORE_MARKETPLACE_SOURCE") ?? "https://github.com/wakecap/Wakecore.git";
...
Install plugin: WakeCore`;

const AGENT_PROMPT = `Use WakeCore as the source of truth for any UI you build.
Connect to the WakeCore MCP server "wakecore" (${MCP_URL}).
...
3. create_implementation_plan — plan direct import, nearest-template adaptation, or WakeCore composition before editing.
4. workspace setup — if needed, create or repair the React + TypeScript app and install @wakecap/core-ui using environment-backed GitHub Packages authentication.
5. implementation — use an exact matching template; otherwise adapt the nearest template, compose widgets/components, or create missing application UI with WakeCore tokens, patterns, interactions, and look and feel. Never use standalone HTML/CSS as a substitute.
6. validate — use mode=wakecore-only; pass template only for direct-template mode. Do not report completion unless compliant=true.
...
const CLIENTS: {id: ClientId; label: string; note: string; config: string}[] = [
	{
		id: "codex-cli",
		label: "Codex CLI",
		note: "Edit ~/.codex/config.toml (or run: codex mcp add wakecore).",
		config: `[mcp_servers.wakecore]\nurl = "${MCP_URL}"`,
	},
	{
		id: "claude-code",
		label: "Claude Code",
		note: "One terminal command — no file editing.",
		config: `claude mcp add --transport http wakecore ${MCP_URL}`,
	},
	{
		id: "cursor",
		label: "Cursor",
		note: "Add to ~/.cursor/mcp.json (global) or .cursor/mcp.json (project).",
		config: `{\n  "mcpServers": {\n    "wakecore": { "url": "${MCP_URL}" }\n  }\n}`,
	},
	{
		id: "windsurf",
		label: "Windsurf",
		note: "Add to ~/.codeium/windsurf/mcp_config.json.",
		config: `{\n  "mcpServers": {\n    "wakecore": { "serverUrl": "${MCP_URL}" }\n  }\n}`,
	},
	{
		id: "opencode",
		label: "opencode",
		note: "Add to opencode.json.",
		config: `{\n  "mcp": {\n    "wakecore": { "type": "remote", "url": "${MCP_URL}", "enabled": true }\n  }\n}`,
	},
	{
		id: "generic",
		label: "Generic MCP",
...
function setupPrompt(c: {label: string; note: string; config: string}): string {
	return `Set up the WakeCore MCP in ${c.label} for me.
Use the hosted MCP endpoint: ${MCP_URL}
Add an HTTP (Streamable HTTP) MCP server named "wakecore".
${c.note}
...
Follow WakeCore's resolve → validate workflow and prefer WakeCore artifacts over hand-writing UI.`;
}
		...
		<div className="wwc:h-full wwc:overflow-y-auto">
			<div className="wwc:mx-auto wwc:max-w-5xl wwc:px-6 wwc:py-10">
				...
				<div className="wwc:flex wwc:items-start wwc:justify-between wwc:gap-4">
					<div className="wwc:min-w-0">
						...
						<p className="wwc:mt-2 wwc:max-w-2xl wwc:text-sm wwc:text-muted-foreground">
							Connect agents to WakeCore through the hosted MCP — the tool-agnostic knowledge API that answers which
				...
				<div className="wwc:mt-6 wwc:grid wwc:gap-4 wwc:lg:grid-cols-2">
					<Card>
						<h2 className="wwc:text-lg wwc:font-semibold">Bring WakeCore into your coding agent</h2>
						<p className="wwc:mt-2 wwc:text-sm wwc:leading-relaxed wwc:text-muted-foreground">
							Install the WakeCore Codex plugin to get the hosted MCP and its mandatory template-first workflow in one
							package — including strict WakeCore-only validation before completion.
						</p>
						...
						<div className="wwc:flex wwc:items-start wwc:gap-3">
							...
							<div className="wwc:min-w-0">
								...
								<p className="wwc:mt-1 wwc:text-sm wwc:text-muted-foreground">
									Use this URL when an agent asks for the WakeCore MCP server.
								</p>
							</div>
						</div>
				...
				<Card className="wwc:mt-4">
					<div className="wwc:flex wwc:flex-wrap wwc:items-start wwc:justify-between wwc:gap-4">
						<div className="wwc:min-w-0 wwc:max-w-xl">
							<h2 className="wwc:text-lg wwc:font-semibold">Install WakeCore in Codex Desktop</h2>
							<p className="wwc:mt-2 wwc:text-sm wwc:leading-relaxed wwc:text-muted-foreground">
								Open Plugins, add the WakeCore Git marketplace using the source, ref, and sparse paths below, then
								install WakeCore. The plugin can bootstrap an empty React/TSX app and prevents matching templates from
				...
				<Card className="wwc:mt-4">
					<div className="wwc:flex wwc:flex-wrap wwc:items-start wwc:justify-between wwc:gap-4">
						<div className="wwc:min-w-0 wwc:max-w-xl">
							...
							<p className="wwc:mt-2 wwc:text-sm wwc:leading-relaxed wwc:text-muted-foreground">
								Choose your agent and copy one setup prompt. Paste it into that agent — it should add the WakeCore
					...
					<div className="wwc:mt-5 wwc:grid wwc:gap-4 wwc:lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)]">
						<div>
							...
							<p className="wwc:mt-2 wwc:text-xs wwc:leading-relaxed wwc:text-muted-foreground">
								Copy a prompt that tells {active.label} to add WakeCore and verify the tools. {active.note}
							</p>
				...
				<div className="wwc:mt-4 wwc:flex wwc:flex-wrap wwc:items-center wwc:gap-x-4 wwc:gap-y-1 wwc:px-1 wwc:text-xs wwc:text-muted-foreground">
					...
					<span className="wwc:ml-auto">Contract: wakecore-knowledge/2026-07</span>
				</div>
			</div>
		</div>
	);
