---
name: core-ui
description: Build, bootstrap, modify, or review React and TSX applications with @core/core-ui. Resolves templates, plans artifact adoption, and validates the resulting multi-file implementation against a goal-specific Core compliance mode.
---

# Core UI

Use this skill for every task that creates, changes, refactors, or reviews a React UI that can use Core. Core is authoritative for layout, templates, widgets, components, imports, tokens, and completion validation.
...
3. `@core/core-ui` installed or available in the workspace dependency graph.

Missing workspace prerequisites are setup work, not an immediate blocker. Continue through template resolution and planning, then prepare the workspace in stage 3. Stop only when the `core` MCP server or a required workflow tool is unavailable; report the missing server or tool and do not approximate its answer from memory.
...
- `product-ui`: build or modify a product interface for normal use. Validate with `core-product`.
- `core-showcase`: demonstrate, test, or evaluate meaningful Core adoption. Validate with `core-showcase`.
- `component-evaluation`: visibly exercise selected components, variants, states, and interactions. Validate with `core-showcase`; relevance is defined by the plan contract, so unrelated widgets are not required.
- `visual-reproduction`: reproduce a selected Core template with strict structural fidelity. Validate with `core-template-strict`.

Do not use the imports-only mode as the completion gate for a product, showcase, or template-reproduction task. `core-only` is a deprecated alias for `core-imports`; use it only when checking a legacy integration that cannot yet send the new mode.
...
Create a structured `referenceAnalysis` map for every image covering product shell, navigation model, sidebar/top-bar density, route identity, visible regions and hierarchy, repeated patterns, scroll ownership, responsive implications, implied interactions, semantic relationships such as parent/back navigation, and source-brand traits that Core must replace. Preserve the reference's information architecture, hierarchy, density, content, and user intent; Core remains authoritative for shell, artifacts, visual language, states, tokens, accessibility, and interaction behavior.

Pass the completed `referenceAnalysis` to `resolve_template` and carry the exact map into `create_implementation_plan`. Every analyzed region must receive a named Core owner or an explicit catalog gap. No implementation file may be edited until this map exists.
...
- `adapt-template`: use the nearest template as a structural and interaction reference, then modify its composition to meet the request with Core artifacts.
- `compose`: proceed without a template and discover the required Core widgets, components, patterns, and utilities.

Follow `selectionGate.fallbackOrder` automatically: `adapt-template` → `compose-widgets` → `compose-components` → `create-with-core`. A low-confidence or no-match result is a routing decision, not a blocker.
...
For an application with more than one route, plan the shared shell before individual screens. Record a route/region map with each route's intent, required regions, required interactions, assigned Core artifacts, and module-boundary ID when applicable. For screenshot work, each region must also name its owner artifact, supported variant, density/state, surface owner, scroll owner, interaction owner, required capabilities, preserved reference traits, and traits intentionally replaced by Core. Record a normative shell fingerprint containing the shell ID, sidebar/top-bar artifact IDs, paired density, brand key, navigation fingerprint, footer fingerprint, provider owner, and allowed route mutations. Selecting either `CoreAppSidebar` or `CoreAppTopBar` requires the other at the same density. Use that stable Core sidebar and top bar across routes unless the plan declares an intentional second-product module boundary with its own complete fingerprint and route list. Route-local branding, navigation, density, or provider swaps are shell drift, not implicit module boundaries. A reference image may guide hierarchy and density, but its brand, palette, and locally reconstructed controls never override Core identity or a catalog artifact unless the user explicitly requires that brand.
...
- If the workspace contains an unrelated project and the user requested a new standalone app, create it in a clearly named subdirectory such as `core-app/` instead of overwriting existing files.
...
- Configure the Core scope without committing a secret:
...
- Install `@core/core-ui`, `@core/core-tokens`, and `@core/core-utils`. Reuse `NODE_AUTH_TOKEN` when it is already exported. When it is absent but the GitHub CLI is authenticated, obtain `gh auth token` only for the scoped install process; never print, log, or write the token value.
- Import `@core/core-ui/styles.css` from the application entry point unless the existing app already uses a documented Core Tailwind integration.
- When `workspaceRequirements.runtimeAudit.required` is true, install its required dev dependencies, add its declared `audit:runtime` package script, provide `scripts/core-runtime-audit.mjs`, and verify the command can start. A missing audit command or harness leaves workspace preparation incomplete.
...
- In `direct-template` mode, use the selected template's exact `@core/core-ui` import/export and preserve its owned layout.
...
- If the catalog has no suitable widget/component, create the missing application-level UI with Core primitives, providers, tokens, spacing, typography, states, accessibility behavior, interaction patterns, and look and feel. This is the final supported fallback, not a reason to block.
- Use Core tokens and components throughout every mode.
...
- Rendering `ZoomTools` over absolutely positioned cards is not a canvas. Use a functional Core canvas/template; when no suitable artifact exists, record a catalog gap and wrap an approved canvas engine with Core nodes, controls, tokens, focus states, and panels.
...
- Mark the rendered shell and route regions with `data-core-shell` and `data-core-region` so the runtime audit can verify continuity and ownership. Expose the planned shell fingerprint using `data-core-artifact`, `data-core-density`, `data-core-brand`, `data-core-navigation-fingerprint`, and `data-core-provider-owner`; mark route scrolling with `data-core-content-scroll`. Mark owned surfaces with `data-core-surface-owner`, duplicate-sensitive controls with `data-core-affordance-purpose`, navigation controls with `data-core-navigation-purpose` and `data-core-navigation-relationship`, and functional canvases with `data-core-canvas-capabilities` plus observable state evidence. Mark planned controls with `data-core-interaction="<interaction-id>"` and at least one internal link per route with `data-core-route-link`. Use router-aware links; do not replace route transitions with `window.location`, forced reloads, or parallel shells.
...
- The imported/rendered/exercised Core inventory.
...
- For screenshot work, report reference fidelity and Core fidelity separately, including intentional differences from the reference.
