...
The **WakeCore Knowledge Platform** — a versioned, transport-agnostic API over WakeCore knowledge.
This is the product; MCP (`wakecore-hosted-mcp`) is one transport over it.
...
| `validate`                   | Is this generated **code** valid WakeCore usage? (authoritative, deterministic) |
...
## Strict WakeCore implementation workflow
...
3. Follow the fallback order: exact template → adapt nearest template → compose widgets → compose components → create application UI with WakeCore primitives, tokens, patterns, and look and feel.
4. If React, TSX, or `@wakecap/core-ui` is missing, prepare the workspace and continue unless setup genuinely fails.
5. Call `validate` with `{mode: "wakecore-only", code: "..."}`; include `template` only for `direct-template` mode.
...
and convergence of `@wakecap/sdk` onto this platform.
