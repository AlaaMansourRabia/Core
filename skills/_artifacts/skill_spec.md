...
**Consumers:** Internal Core product teams building features with the design system
...
1. Install and configure all three Core packages correctly from the first try
...
Core has two independently published chart systems, a standalone token package,
...
|---|---|---|---|
| `core-ui-quickstart` | `@core/core-tokens` | lifecycle | `packages/tokens/skills/core-ui-quickstart/SKILL.md` |
| `core-ui-components` | `@core/core-ui` | core | `packages/components/skills/core-ui-components/SKILL.md` |
| `core-ui-forms` | `@core/core-ui` | core | `packages/components/skills/core-ui-forms/SKILL.md` |
| `core-ui-data-table` | `@core/core-ui` | core | `packages/components/skills/core-ui-data-table/SKILL.md` |
| `core-ui-charts` | `@core/core-ui` | core | `packages/components/skills/core-ui-charts/SKILL.md` |
| `core-ui-chat` | `@core/core-ui` | core | `packages/components/skills/core-ui-chat/SKILL.md` |
| `core-tokens` | `@core/core-tokens` | core | `packages/tokens/skills/core-tokens/SKILL.md` |
| `core-utils` | `@core/core-utils` | core | `packages/utils/skills/core-utils/SKILL.md` |
...
Core is a private design system published to GitHub Packages. Consumers need
`.npmrc` with `@core:registry=https://npm.pkg.github.com` before installation.

All Tailwind utility classes in Core use the `wwc:` prefix (from
`@import "tailwindcss" prefix(wwc)` in core-tokens). Consumer code that uses
`cn()` from `@core/core-utils` or passes `className` props to Core
...
- **Pre-built CSS** (`@import "@core/core-ui/styles.css"`) — no Tailwind required
- **Tailwind 4** — import `@core/core-tokens` + `@source` the component dist
- **Tailwind 3** — use pre-built CSS alongside existing TW3 (no conflicts due to `wwc:` prefix)

The `@core/core-ui` barrel (`import from "@core/core-ui"`) only exports
types, mock-data, and chatService. All components use deep-path imports:
`import { X } from "@core/core-ui/x"`.

`@core/core-tokens` is CSS-only with no JavaScript — import it once at the
