...
import {Toaster, toast} from "@corensystem/coren-ui/sonner";
		...
		docs: {
			description: {
				component:
					...
					"root and call `toast()` — both from `@corensystem/coren-ui/sonner`. Related: `Alert`, `Banner`, `Toast` (lower-level).",
			},
		},
	},
		...
		docs: {
			description: {
				story:
					...
					"`toast()`. Both come from `@corensystem/coren-ui/sonner`. In these stories a decorator mounts the `<Toaster/>` for you.",
			},
		},
	},
	render: () => (
		<div className="wwc:flex wwc:flex-col wwc:gap-3 wwc:max-w-md">
			<pre className="wwc:text-[12px] wwc:bg-muted wwc:rounded-md wwc:p-3 wwc:overflow-x-auto">
				{`// app root (once)
import { Toaster } from "@corensystem/coren-ui/sonner";
<Toaster />
...
export const CommonMistake: Story = {
	parameters: {
		docs: {
			description: {
				story:
					"**Avoid: mixing toast systems.** `toast()` from `@corensystem/coren-ui/sonner` is rendered only by the Sonner " +
					"`<Toaster/>` from the *same* package. Mounting the lower-level Radix `Toaster` (from `@corensystem/coren-ui/toaster`) " +
