...
- commit them to `wakecap/Wakecore` (every clone would balloon), or
- ship them inside the `@wakecap/core-ui` npm package (every consumer would download them).
...
**`wakecap/wakecore-3d-assets`** (visibility: **internal** — visible to wakecap members, not public).
...
1. **Create a token** with read access to `wakecap/wakecore-3d-assets` (a fine-grained PAT scoped to that
...
2. **Add it as a secret** on `wakecap/Wakecore` (or the org): `ASSETS_REPO_TOKEN`.
...
the GitHub CLI with access to `wakecap/wakecore-3d-assets` (`gh auth login`), install "just works". The fetch
...
- clone `wakecap/wakecore-3d-assets` and copy/symlink each mockup folder into `apps/storybook/public/`
...
1. Push the heavy files to `wakecap/wakecore-3d-assets` under a new mockup folder (see its README).
