...
> **Core is the source of truth for UI. AI editors do the work. Studio is the read‑only front door to Core.**

Studio does not edit, does not converse, does not run an agent, and **does not reflect personal or unapproved edits.** It renders only what the Core repository has already approved and published. Changes reach Studio through the normal contribution flow (commit → PR → review → merge → publish), never by watching a local workspace.
...
- **Core solves knowledge, not editing.** It knows templates, widgets, components, tokens, composition rules, validation, and governance. Products like Claude Code, Cursor, Codex, Windsurf, Lovable, Bolt, and v0 already solve editing well — Core does not compete with them.
- **The Core repository is the single source of truth.** Not the generated workspace, not the editor, not Studio's cache.
- **Studio is canonical and read‑only.** It visualizes the current, approved state of Core and nothing else.
...
```
┌──────────────────────────┐        ┌───────────────────────────┐        ┌──────────────────────────┐
│     CORE STUDIO       │  read  │     CORE REPOSITORY     │  PR   │    EXTERNAL WORKSPACE     │
...
|---|---|---|
| **Core Studio** | browse, preview canonical artifacts, launch an editor into a fresh workspace | edit, converse, stream, run an agent, watch a workspace, display unapproved edits |
...
| **Core repository** | review, approve, promote, publish; hosts every canonical artifact | — |
| **Editor** (Claude/Cursor/…) | conversation, agent loop, editing, compiling, fixing, tools | owns no Core knowledge (it *reads* what the workspace provides) |
| **Core knowledge** (SDK/MCP/renderer) | templates, widgets, tokens, manifests, validation, **the renderer** | — |
...
Browse official template
      ↓
Preview official template          ← Core renderer, read‑only, no disk written
      ↓
Edit in Claude Code                ← user commits to editing
      ↓
Pick a workspace location
      ↓
Materialize a separate workspace   ← src/ + .core/ + editor config
      ↓
...
Commit / branch / PR               ← the normal contribution flow
      ↓
Review → approve → merge → publish ← canonical Core updates
      ↓
...
│  │   folder only)│ ─────────────▶ │ Preview service ─▶ Core Renderer   │    │
...
└─────────┼─────────────────────────────────────────────────────────────────────┼─┘
   ...
   ┌──────┴───────────────┐                                    ┌─────────────────▼─┐
   │ CORE REPOSITORY  │   approved changes (PR → merge)     │ EXTERNAL WORKSPACE│
   │ templates · widgets  │◀───────────────────────────────────│ Claude Code edits │
   │ tokens · manifests   │                                    │ src/  (private)   │
   │ SDK · MCP · renderer  │   reads .core/ + MCP ─────────▶│ .core/ (ctx)  │
   └──────────────────────┘                                    └───────────────────┘
```

Two seams only: **Studio → Editor** = `editor.launch(workspace)`; **Editor/workspace → Core** = `.core/` + SDK/MCP. Studio reads the repository; the workspace proposes to the repository; **Studio and the workspace never talk.**
...
<workspace>/
  src/                      ← the editable app source (the editor owns this)
    page.tsx
  .core/                ← canonical, editor-agnostic Core context FOR THE EDITOR
    workspace.json          ← id, name, template id, created, schemaVersion
    template.json           ← snapshot of the source template's manifest
    page-instance.json      ← the Core composition SEED + validation target (advisory)
    artifacts.json          ← locally generated candidates awaiting promotion (future)
  CLAUDE.md                 ← SMALL & STABLE: "use .core/ + the Core SDK & MCP"
  AGENTS.md                 ← same policy, cross-editor convention
  .mcp.json                 ← the core MCP server
  .claude/settings.json     ← tool allowlist + permissions
  (package.json, vite.config.ts … only when we add Run/Export — see §11 Risk 3)
```

- **`.core/` is context for the editor, not for Studio.** Studio writes it at materialize time and never reads it afterward.
- **`CLAUDE.md` stays ~15 lines and is never regenerated.** The *data* lives in `.core/`; the *instructions* stay generic (point the AI at `.core/` + SDK + MCP, reuse Core, import from `@core/core-ui/<kebab>`).
- `page-instance.json` is a **seed + validation target**, not live truth — the editor edits `src/`; `.core/` is regenerated/validated via the SDK, never hand-authored (see §11 Risk 2).
...
click "Edit in Claude Code"
  └─▶ POST /api/workspace/materialize { templateId, location }
        ...
        3. write .core/ (workspace.json, template.json, page-instance.json)   ← from the SDK
        4. write generic CLAUDE.md / AGENTS.md / .mcp.json / .claude/settings.json
        5. editor = registry.get("claude")
        6. editor.prepare(workspace)     ← project .core/ into THIS editor's config (idempotent)
...
interface Editor {
  ...
  prepare(workspace): void     // project .core/ into THIS editor's expected config files
...
Studio's "refresh" therefore means **re-read the repository's canonical outputs** (`library-index.json`, `manifests/`, built templates, Storybook), not "watch a workspace." In local dev this is a catalog reload on repository change; in a published setting it follows the release of the Core packages. **Unapproved edits never appear.**
...
- **Phase 2 — Split preview from workspace.** `/api/build` (which created a workspace and compiled) becomes **`/api/preview`** — render a canonical template with the Core renderer, **no disk written**. The Preview page shows the running canonical template + one **Edit** action. No workspace yet.
- **Phase 3 — Workspace materializer + `.core/` schema.** `server/workspace.ts` → materialize a real, external workspace *only on Edit*: `src/` + `.core/` (versioned schemas) + thin editor config. Drop `sessionId`/`history`. Studio writes it and never reads it back.
...
|---|---|---|---|
| Catalog (`server/catalog.ts`, `lib/catalog.ts`), `TemplateThumb`, Home grid/search, **the Core renderer + esbuild compile as the canonical preview**, theme | `server/agent.ts`, `/api/message`, `sendMessage`+`AgentEvent`, `lib/models.ts`, the chat rail in `BuildScreen`, the repair loop, warm sessions, attachment→CLI plumbing, **any watcher / HMR / workspace-iframe / edit-refresh** | `Home.tsx` (drop chat composer/model/attachments → search + grid + recent), `App.tsx` (Home ↔ canonical Preview), `build.ts` (→ `/api/preview` only) | `server/workspace.ts` → workspace materializer (drop `sessionId`/`history`); the Core-first **policy** → from agent spawn-flags into `.core/` + a small stable `CLAUDE.md`; template seeds → workspace `src/` |

---

## 12. Risks & alternatives

**Risk 1 — the renderer boundary (now largely resolved).** Because Studio renders **only canonical templates** and never edited workspace code, the Core renderer never has to represent arbitrary React. The earlier "preview silently lies about edited code" failure mode is gone by construction. Remaining nuance: the renderer must faithfully render *canonical* artifacts (templates/widgets/components) — that is squarely a Core responsibility and already exists (the vendored renderer used by the current preview). ✅

**Risk 2 — `page-instance.json` vs code truth.** `src/page.tsx` is the working truth the *editor* owns; `.core/page-instance.json` is a **seed + validation target**, regenerated/validated via the SDK and allowed to drift from code. Do not attempt a code↔instance round-trip — it keeps Core *advisory* (its strength) and keeps governance sane (validation compares code against intent; it does not try to *be* the code).

**Risk 3 — don't scaffold a runnable app before it's needed.** Since Studio's preview is the Core renderer (not the workspace's dev server), the workspace need not be independently runnable to exist. Materialize the **minimal** workspace first (`src/` + `.core/` + editor config); make it a full runnable app only when we add **Run/Export**, and resolve `@core/core-ui` via a shared/linked store rather than per-workspace installs. Keeps disk light and defers the dependency question.

**Risk 4 — "editor-agnostic" requires per-editor config generation.** Different editors read different files (`CLAUDE.md` / `.cursor/rules` / `AGENTS.md`). `.core/` is canonical; each `Editor.prepare()` projects it into that editor's format at launch. This is what makes "tomorrow, another editor" actually true.
