#!/usr/bin/env node
// Wire WakeCore into an Open Design instance: (1) install the wakecore-compose skill so its workflow
// is injected into generation, and (2) register the WakeCore MCP server so OD's agent can call the
// WakeCore tools. Idempotent. Env overrides: OD_DIR (OD checkout), OD_DATA_DIR (OD runtime state).
...
const WAKECORE_ROOT = join(HERE, "..", "..", ".."); // repo root
const OD_DIR = process.env.OD_DIR ?? "/tmp/open-design";
const OD_DATA_DIR = process.env.OD_DATA_DIR ?? "/tmp/od-data";

// 0) patch OD's Home so its NATIVE template rail + picker ARE the WakeCore templates (click → build,
...
// the reusable MCP + skill + gallery plugin below work without it.
{
	...
	if (applied()) {
		...
		try {
			...
			console.log("✓ home-templates patch applied (OD native rail = WakeCore templates)");
		} catch (e) {
...
const skillDir = join(OD_DIR, "skills", "wakecore-compose");
mkdirSync(skillDir, {recursive: true});
copyFileSync(join(HERE, "SKILL.md"), join(skillDir, "SKILL.md"));
console.log(`✓ skill installed → ${skillDir}/SKILL.md`);

// 1b) install the "WakeCore Templates" gallery plugin (a scenario) into OD's auto-scanned scenarios
// dir so it appears in the Plugins gallery — click a template → generate via WakeCore → open in the
// editor. OD scans plugins/_official/scenarios/** from disk on startup (requires an OD restart).
const galleryDir = join(OD_DIR, "plugins", "_official", "scenarios", "wakecore-templates");
...
// 2) register the WakeCore MCP server in OD's external mcp-config
...
const server = {
	...
	enabled: true,
	command: "node",
	args: [join(WAKECORE_ROOT, "packages", "mcp", "src", "server.mjs")],
};
cfg.servers = cfg.servers.filter((s) => s.id !== "wakecore");
cfg.servers.push(server);
writeFileSync(cfgPath, JSON.stringify(cfg, null, 2));
console.log(`✓ MCP server registered → ${cfgPath}`);
console.log(`  wakecore → node ${server.args[0]}`);

console.log(`
Next:
  1. Start the WakeCore renderer:   node ${join(WAKECORE_ROOT, "integrations/open-design/renderer/serve.mjs")}
  2. Start Open Design:             cd ${OD_DIR} && OD_DATA_DIR=${OD_DATA_DIR} pnpm tools-dev run web
  3. In OD: new project → pick the "wakecore-compose" skill → prompt e.g. "an operations dashboard".
`);
