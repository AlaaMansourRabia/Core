...
import {ObjectDrawingToolbar} from "@/components/ui/object-drawing-toolbar";
...
/** Build a shape from a drag box (start → current) for the given tool. */
function makeShape(id: string, tool: DrawingTool, a: Point, b: Point, snap: boolean, style: Style): Shape | null {
	if (tool === "line") {
		let [x2, y2] = b;
		if (snap) {
			// Snap to the nearest horizontal / vertical axis.
	...
	if (snap) {
		// Constrain to a square / circle bounding box.
	...
	if (tool === "polygon") {
		// A regular pentagon inscribed in the drag box.
		const points: Point[] = [];
		for (let i = 0; i < 5; i++) {
...
/** Whether a drag produced something big enough to keep (avoids stray click dots). */
...
/** Axis-aligned bounding box, used for the selection outline. */
...
export function ObjectDrawingCanvasPage() {
	// Panel collapse/expand — same mechanism as the Design Canvas example.
	...
	// Toolbar-mirrored drawing settings.
	...
	// Live values for window-level pointer handlers (avoids stale closures).
	...
	function onSurfacePointerDown(e: React.PointerEvent) {
		...
		if (active === "select") {
			// Background click clears selection; shape clicks are handled on the shape.
			setSelectedId(null);
			return;
		}
	...
	function onShapePointerDown(e: React.PointerEvent, id: string) {
		const {editing: ed, tool: t} = live.current;
		if (!ed || t !== "select") return; // let it bubble to the surface (draw / pan)
		e.stopPropagation();
		setSelectedId(id);
		gesture.current = {kind: "move", id, last: toWorld(e)};
	}

	// Window-level move / up so gestures survive the pointer leaving the SVG.
	useEffect(() => {
		...
		<div className="wwc:relative wwc:h-full wwc:w-full wwc:overflow-hidden wwc:bg-muted/30">
			...
			>
				...
				<g transform={`translate(${pan.x} ${pan.y})`}>
					{/* Large panning grid so the surface feels infinite. */}
							...
							<li key={s.id}>
								...
								>
									<span
										...
										style={{backgroundColor: s.fill || "transparent", borderColor: s.stroke || "#d4d4d8"}}
									/>
	...
	const shell = (
		<div
			className={`wwc:flex wwc:flex-col wwc:overflow-hidden wwc:rounded-lg wwc:border wwc:bg-background ${
				fullscreen ? "wwc:h-full" : "wwc:h-[640px]"
			}`}
		>
			{/* Top bar: panel toggles + the Object Drawing Toolbar */}
			...
			{/* Body: collapsible layers | canvas | properties */}
	...
	if (fullscreen) {
		return <div className="wwc:fixed wwc:inset-0 wwc:z-50 wwc:bg-background wwc:p-4">{shell}</div>;
	}
			...
			<div className="wwc:flex wwc:items-center wwc:justify-between wwc:gap-4">
				<div>
					<div className="wwc:group wwc:flex wwc:items-center wwc:gap-3">
						<h1 className="wwc:text-3xl wwc:font-bold">Object Drawing Canvas</h1>
						<CopyButton
							value="Object Drawing Canvas"
							className="wwc:opacity-0 wwc:group-hover:opacity-100 wwc:focus-visible:opacity-100"
						/>
					</div>
					<p className="wwc:mt-2 wwc:text-muted-foreground">
						A functional drawing surface driven by the{" "}
						<code className="wwc:rounded wwc:bg-muted wwc:px-1 wwc:text-xs">ObjectDrawingToolbar</code>. Pick a shape
						tool and drag to draw; use Select to move objects, Pan (or View mode) to move the canvas. The collapsible
						Layers and Properties panels open and close from the toolbar or by dragging their handle.
					</p>
				</div>
