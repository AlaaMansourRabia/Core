...
test("core-imports remains a compatibility-level import check even with authored CSS", async () => {
		...
		files: [
			{
				path: "src/App.tsx",
				language: "tsx",
				content: 'import {Button} from "@corensystem/coren-ui/button"; export default () => <Button>Go</Button>;',
			},
...
test("product validation detects a manually recreated standard sidebar", async () => {
	const result = await kb.callTool("validate", {
		...
		implementationPlan: productPlan,
		code: `
			import {Sidebar, SidebarHeader, SidebarContent, SidebarMenu, SidebarFooter} from "@corensystem/coren-ui/sidebar";
		...
		files: [
			{
				...
				language: "tsx",
				content: `
					import {CoreAppSidebar} from "@corensystem/coren-ui/navigation/core-app-sidebar";
					...
					import {CoreContentArea} from "@corensystem/coren-ui/pages/core-content-area";
...
test("core-only reports its deprecation alias", async () => {
	const result = await kb.callTool("validate", {
		mode: "core-only",
		code: 'import {Button} from "@corensystem/coren-ui/button"; export default () => <Button>Go</Button>;',
	});
