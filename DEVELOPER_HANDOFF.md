# Core — GitHub OAuth Org-Gating: Developer Handoff

**Repo:** `AlaaMansourRabia/Core`  ·  **Owner:** Alaa Mansour
...
source check of `core/Core` shows **the consuming backend has not been built on any branch**
...
| Session cookie + `core` org-membership check | ❌ None |
...
> **Stack reality:** Core is a **Vite SPA** (`apps/web/dist/index.html`) plus Vercel serverless
...
| Denial message (exact copy) | `Access denied. You must be a member of the core organization.` |
...
- `302` → `https://github.com/login/oauth/authorize` with:
  `client_id=$GITHUB_CLIENT_ID`, `redirect_uri=https://core.core.com/api/auth/callback`,
...
3. **Org check** with the user token:
   `GET https://api.github.com/user/memberships/orgs/core` (header `Authorization: Bearer <token>`).
...
| 5 | `curl https://core.core.com/health` | Returns JSON, **no redirect** (MCP stays open) |
...
- [ ] Register prod OAuth App (org `core`) — homepage + callback per §2 → capture Client ID + secret
