					...
					<CommandList>
						{filtered.length === 0 ? (
							<div className="wwc:py-6 wwc:text-center">
								...
								<p className="wwc:mt-2 wwc:text-sm wwc:text-muted-foreground">No users match "{search}"</p>
								<p className="wwc:text-xs wwc:text-muted-foreground">Try a different search term</p>
							</div>
						) : (
							<CommandGroup>
								{filtered.map((user) => (
