...
export function TaskMonitorExamplePage() {
	...
	return (
		...
		>
				...
				<div className="wwc:flex wwc:items-center wwc:justify-between wwc:gap-4">
					<div>
						<div className="wwc:group wwc:flex wwc:items-center wwc:gap-3">
							<h1 className="wwc:text-3xl wwc:font-bold">Task Monitor — Fixed Content #2</h1>
							<CopyButton
								value="Task Monitor — Fixed Content #2"
								className="wwc:opacity-0 wwc:group-hover:opacity-100 wwc:focus-visible:opacity-100"
							/>
						</div>
						<p className="wwc:text-muted-foreground wwc:mt-2">
							The Task Monitor spec realised on the Fixed Content #2 layout. Left = task list with search / filter /
							kebab, middle = browser-style tabs + viewer + comment composer, right = actionable details + grouped
							actions. Both side panels are collapsible.
						</p>
					</div>
			...
			<div
				className={cn(
					"wwc:overflow-hidden wwc:flex",
					fullscreen ? "wwc:h-screen" : "wwc:h-[640px] wwc:rounded-xl wwc:border",
				)}
			>
							...
							trailing={
								<DropdownMenu>
									<DropdownMenuTrigger asChild>
										<Button variant="ghost" icon className="wwc:h-8 wwc:w-8" aria-label="List actions">
											<MoreVertical className="wwc:h-4 wwc:w-4" />
										</Button>
									</DropdownMenuTrigger>
									<DropdownMenuContent align="end">
										<DropdownMenuItem onSelect={() => setTasks((prev) => prev.filter((t) => t.status !== "done"))}>
											<Archive className="wwc:mr-2 wwc:h-4 wwc:w-4" />
											Archive completed
										</DropdownMenuItem>
										<DropdownMenuItem
