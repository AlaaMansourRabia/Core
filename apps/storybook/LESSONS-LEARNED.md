...
The manifest generator uses `tsconfig-paths` + Node.js `resolve` to locate component source files for prop type extraction. In a monorepo where stories import from the package name (e.g., `@wakecap/core-ui/accordion`), the default resolution lands on `dist/*.mjs` — compiled files that react-docgen can't parse.
...
```json
{
	"compilerOptions": {
		"baseUrl": "../..",
		"paths": {
			"@wakecap/core-ui/*": ["packages/components/src/*"]
		}
	}
}
```
...
The design tokens use `.dark` class selector for CSS variable overrides, but Tailwind 4 `dark:` utility classes won't activate without declaring the variant. Add this to `@wakecap/core-tokens` (not per-app) so all consumers get it:
...
```ts
addons.register("wakecap/theme-sync", (api) => {
	const channel = api.getChannel();
	channel.on("updateGlobals", () => {
		const theme = api.getGlobals()?.theme || "light";
		addons.setConfig({
			theme: theme === "dark" ? wakecapDark : wakecapLight,
		});
	});
});
```
...
The Storybook manager UI runs outside the preview iframe, so it does not inherit fonts from `preview.css` or `@wakecap/core-tokens`. Import Google Fonts directly in `manager.css`:
...
Wakecore uses Tailwind CSS 4's `prefix(wwc)` feature. Every utility class **inside component source files** is prefixed: `wwc:flex`, `wwc:bg-primary`, `wwc:hover:text-foreground`. This prevents collisions when consumers have their own Tailwind setup.

The prefix is an **internal implementation detail** — consumers never write `wwc:` in their own code. Components are self-styled black boxes. For TW3 consumers, `@wakecap/core-tokens/tailwind3-preset` maps design tokens to unprefixed TW3 utilities.

The prefix is configured in `@wakecap/core-tokens`:
...
The `cn()` utility in `@wakecap/core-utils` uses `extendTailwindMerge` with the prefix so it correctly resolves conflicts like `wwc:p-4 wwc:p-6` → `wwc:p-6`.
...
All page components accept typed props (`Organization`, `Project`) from `@wakecap/core-ui/types`. Mock data is centralized in `@wakecap/core-ui/data/mock-data`:

```tsx
import {MOCK_ORGANIZATIONS, MOCK_PROJECTS} from "@wakecap/core-ui/data/mock-data";
