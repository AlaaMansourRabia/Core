# WakeCore Codex plugin

This repository-owned plugin connects Codex to the production WakeCore MCP at
`https://core.wakecap.com/mcp` and installs the `wakecore-ui` skill. The skill makes the following
...
An exact direct-template match must be imported from `@wakecap/core-ui`; it cannot be silently replaced
by standalone HTML/CSS. Low-confidence and no-match results do not block. The skill automatically
adapts the nearest template, composes WakeCore widgets/components, or creates missing application-level
UI with WakeCore primitives, tokens, interaction patterns, and look and feel. For multi-route work, the
...
also scaffolds React and TypeScript, installs WakeCore, implements, validates, and runs the app.
...
and interactions while identifying source-brand traits that WakeCore must replace. Each region is then
bound to a WakeCore owner or an explicit catalog gap. Runtime audit v4 verifies contiguous shell geometry,
...
WakeCore fidelity separately.

Validation modes describe the claim being made:

- `wakecore-imports`: import-level integration checks.
- `wakecore-product`: normal product UI adoption.
- `wakecore-showcase`: meaningful WakeCore demonstration or evaluation.
- `wakecore-template-strict`: strict reproduction of a selected template.

`wakecore-only` remains a deprecated alias for `wakecore-imports`; it must not be used to claim product,
...
python3 "$CODEX_HOME/skills/.system/plugin-creator/scripts/validate_plugin.py" plugins/wakecore
codex plugin marketplace add .
codex plugin add wakecore@wakecore
```
...
`codex plugin list` to verify the `wakecore` marketplace and plugin are present.
...
`@wakecap/core-ui` is distributed through GitHub Packages. Workspace bootstrap writes only the
...
If WakeCore enables bearer authentication, configure the server with Codex's environment-backed
...
```json
{
	"mcpServers": {
		"wakecore": {
			"type": "http",
			"url": "https://core.wakecap.com/mcp",
			"bearer_token_env_var": "WAKECORE_MCP_TOKEN"
		}
	}
}
```
...
```bash
export WAKECORE_MCP_TOKEN="<value-from-the-WakeCore-team>"
```

Do not use a literal bearer token or static `Authorization` value. For OAuth, use Codex's MCP login
flow after WakeCore publishes OAuth metadata instead of committing credentials.
...
```bash
python3 "$CODEX_HOME/skills/.system/plugin-creator/scripts/update_plugin_cachebuster.py" plugins/wakecore
pnpm validate:codex-plugin
codex plugin add wakecore@wakecore
```
...
1. Confirm `https://core.wakecap.com/health` returns `wakecore-knowledge`.
2. Confirm the plugin is installed and enabled in the desktop plugin directory.
3. Start a new task and ask Codex to build a React screen that matches a WakeCore template.
...
5. Repeat in an empty fixture with GitHub Packages access; verify the task creates the React/TSX app,
   installs WakeCore, implements the selected template, and runs it locally.
...
7. Use a screenshot with no confident template match; verify implementation continues without a
   confirmation prompt, uses the nearest WakeCore structure or catalog components, and validates
...
8. Verify the final report distinguishes imported, rendered, and exercised inventory; includes detailed
   ...
   `wakecore-imports` compliance to a stronger claim.
